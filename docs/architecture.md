...
- 整屏截图能力测试复用同一私有参考图与视觉请求链路，返回不修改的上游原始 JSON、确定性归一化的解析值和共享契约问题。解析层先完成确定性修正、再校验修正后的结果：按坐标重排行优先顺序，为 `kpi-1` 补齐不影响业务含义的空单位与零趋势展示字段，在背景已明确为三维/视频时移除重复输出的场景控制栏，将同列紧贴、名称同源且主体标题为空的游离标题并入主体，并纠正 `notes` 明确自述为仪表盘且具备多项设备状态证据的 KPI 类型；当 gauge 父边界完整包含已独立拆出的下方 `kpiList` 时，以第一个列表顶边为证据收缩 gauge 底边，不根据文字或任意比例猜测图表拆分坐标。页面级视觉底层写入可选 `canvas.backgroundLayer` 诊断字段，按 `image / interactiveScene / video / unknown` 区分静态图片、三维/GIS、视频和不确定背景；背景层不进入组件排序和重叠检查，也不携带 URL、Base64 或永久资产。组件识别推荐不超过 20 项，超过时提示人工检查，硬上限为 32 项；共享校验覆盖背景层字段、场景控制重复识别、游离标题、KPI 与图表互相误并、数量、阅读顺序、组件大面积重叠、标题来源和系列数语义。诊断结果不进入 `screenStore`、历史栈或 MongoDB，也不能作为组件定义渲染。
- 前端从解析值复制生成唯一的本地结构草稿，参考图覆盖框和编辑列表共用该草稿状态；名称、类型、包含状态和边界修改即时同步。确认只设置面板内状态，上传新图、重新识别、删除图片或关闭面板时统一清理。
- 已确认草稿通过纯函数转换为页面骨架预览：按当前大屏画布计算等比包含与居中偏移，映射到固定内置模板；优先投影合法主题和 `mockData`，旧结果缺少模拟数据时仅从组件内可见的简单数值文字推导 KPI、名值图和表格，无法推导才克隆模板演示数据。告警处理率的名值数据若仅返回百分比，但 `visibleTexts` 已明确给出新增/已解决/未解决计数，页面投影层按相邻可见文字确定性恢复计数，避免将 80% 误显示为 80 条。确认添加时由 `screenStore` 一次性分配页面和组件 id、追加页面并压入单条历史；预览不入 store，正式写入不触发保存接口。
- `unsupported` 与缺失模板的项进入跳过清单；`backgroundLayer` 只进入警告，不写入临时图片或交互场景资产。撤销/重做后统一校正 `currentPageId`，避免新增页面被撤销后仍指向已删除页面。
- 整屏识别的视觉迁移使用两级声明式数据：`canvas.appearance` 保存公共页面主题，`component.appearance` 只保存局部覆盖；前端将两者合并后投影到注册表安全字段。常规功能图标只接受 shared 白名单内的 Lucide 名称，`auto` 在 KPI renderer 内按指标名称确定性映射，不接受 SVG、URL 或 HTML。`mockData` 必须通过组件类型对应的既有数据协议才可写入实例，否则回退模板默认数据并记录生成警告。
- 周期切换、日期范围、右侧操作文字和静态演示 tooltip 作为 renderer 的受控附属展示字段写入实例 style；不建立事件、不请求接口，也不拆成独立组件。旧识别结果可从 `visibleTexts` 确定性恢复常见周期、日期和操作文字，tooltip 只接受模型明确输出。
