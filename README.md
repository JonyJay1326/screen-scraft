...
- **AI 智能设计**：编辑器内通过自然语言和参考图生成安全样式方案；方案先预览、后应用
...
7. **管理后台（管理员）** → 用户管理、DeepSeek 文本/视觉模型配置与能力测试
...
| `LLM_BASE_URL / LLM_API_KEY / LLM_TEXT_MODEL / LLM_VISION_MODEL` | DeepSeek 默认配置（也可在管理后台覆盖，Key 不回显明文） |
| `LLM_CHAT_MODEL / LLM_EMBEDDING_MODEL` | v0.3 AI 客服兼容字段，新功能不使用 |
